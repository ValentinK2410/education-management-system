...
@section('title', __('messages.institutions'))
@section('page-title', __('messages.institutions'))

@push('styles')
<style>
    /* Темная тема для страницы учебных заведений */
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title mb-0">
                        <i class="fas fa-university me-2"></i>{{ __('messages.institutions') }}
                    </h3>
                    <a href="{{ route('admin.institutions.create') }}" class="btn btn-primary">
                        <i class="fas fa-plus me-2"></i>{{ __('messages.add_institution') }}
                    </a>
                </div>
                <div class="card-body">
                                            ...
                                            <div class="d-flex align-items-center">
                                                @if($institution->logo)
                                                    <img src="{{ asset('storage/' . $institution->logo) }}"
                                                         alt="{{ __('messages.logo') }}" class="me-3" style="width: 40px; height: 40px; object-fit: cover; border-radius: 50%;">
                                                @else
                                                ...
                                                <div>
                                                    ...
                                                    <small class="text-muted">{{ $institution->short_name ?? __('messages.no_short_name') }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="badge bg-info">{{ $institution->type ?? __('messages.not_specified') }}</span>
                                        </td>
                                        <td>
                                            <div class="text-truncate" style="max-width: 200px;" title="{{ $institution->address }}">
                                                {{ $institution->address ?? __('messages.not_specified') }}
                                            </div>
                                        </td>
                                        <td>
                                            <div>
                                            ...
                                            <div class="btn-group" role="group">
                                                ...
                                                <form action="{{ route('admin.institutions.destroy', $institution) }}"
                                                    ...
                                                    <button type="submit" class="btn btn-sm btn-danger"
                                                            ...
                                                            onclick="return confirm('{{ __('messages.confirm_delete_institution') }}')">
                                    ...
                                    <tr>
                                        <td colspan="7" class="text-center py-4">
                                            <div class="text-muted">
                                                ...
                                                <p>{{ __('messages.institutions_not_found') }}</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
