...
```bash
cd /path/to/education-management-system
sudo scripts/setup-auto-renewal.sh
```

Скрипт автоматически:
- Проверит текущие задачи cron
- Добавит задачу автоматического обновления
- Выполнит тестовое обновление (dry-run)
...
2. **Если задача отсутствует, добавьте её:**
   ...
   Это будет проверять и обновлять сертификаты дважды в день (в 00:00 и 12:00).
...
- Certbot автоматически проверяет сертификаты дважды в день
- Обновление происходит только если сертификат истекает в течение 30 дней
- После обновления Nginx автоматически перезагружается
- Все действия логируются в `/var/log/letsencrypt/letsencrypt.log`
